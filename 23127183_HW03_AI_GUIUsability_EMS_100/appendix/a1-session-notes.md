# Phụ lục A1 — Ghi chú 5 phiên user testing (dữ liệu thô)

**Sinh viên:** Phạm Vũ Ngọc Duy (23127183) · **Kịch bản:** B — User đăng ký tham dự sự kiện
**Báo cáo phân tích:** [`02-usability-report.md`](../02-usability-report.md) · **Chấm SUS:** [`a2-sus-scoring.md`](a2-sus-scoring.md)

> Đây là **dữ liệu phiên thô** theo đề §12 — lời nói giữ **nguyên văn**, không làm mượt, không tóm tắt.
> Transcript và bản ghi màn hình: [`../evidence/task2/`](../evidence/task2/)

---

## Lời thoại chuẩn (đọc nguyên văn cho cả 5 người để đồng nhất)

**Mở đầu:**
> "Mình đang test **sản phẩm này**, không phải test bạn — bạn không thể làm sai được. Trong lúc làm, bạn nói to những gì đang nghĩ giúp mình nhé: bạn đang tìm gì, bạn mong điều gì xảy ra, chỗ nào làm bạn phân vân. Nếu bạn im lặng lâu mình sẽ nhắc. Mình sẽ không trả lời câu hỏi trong lúc bạn làm, để xem hệ thống tự giải thích được đến đâu."

**Khi participant hỏi:** _"Bạn nghĩ nó sẽ làm gì?"_ — không giải thích.
**Khi participant im > 15 giây:** _"Bạn đang nghĩ gì vậy?"_
**Ngưỡng can thiệp:** bí hoàn toàn quá **2 phút** ở cùng một chỗ → gợi ý mức tối thiểu, **ghi lại là đã can thiệp**.

---

## Phiên P1 — _(TODO tên, ngày giờ)_

| Mục | Giá trị |
|---|---|
| Thiết bị & trình duyệt | _(TODO)_ |
| Sự kiện được giao | _(còn chỗ / hết chỗ + waitlist)_ |
| Task success | Completed / Partial / Failed |
| Time on task | _ph _giây |
| Số error | |
| Số hesitation | |
| Số lần moderator can thiệp | |
| Bản ghi màn hình | `../evidence/task2/P1-recording` |

### Dòng thời gian

| Thời điểm | Hành động | Lời nói (NGUYÊN VĂN) | Phân loại | Màn hình |
|---|---|---|---|---|
| 00:00 | | | | |
| | | | error / hesitation / frustration / success | B2 / B3 / B4 |

### Trả lời câu hỏi mở

| # | Chủ đề | Câu trả lời (nguyên văn) |
|---|---|---|
| Q1 | Clarity | |
| Q2 | Error recovery | |
| Q3 | Speed | |
| Q4 | Trust | |
| Q5 | Tổng quát | |

### Quan sát của moderator

_(TODO — nhận xét sau phiên, tách bạch với dữ liệu quan sát ở trên)_

---

## Phiên P2 — _(TODO)_
_(cấu trúc như P1)_

## Phiên P3 — _(TODO)_
## Phiên P4 — _(TODO)_
## Phiên P5 — _(TODO)_

---

## Pilot — người thứ 6 (KHÔNG tính vào 5 người)

| Mục | Nội dung |
|---|---|
| Người pilot | _(TODO)_ |
| Ngày | _(TODO)_ |
| Vấn đề phát hiện ở kịch bản | _(TODO)_ |
| Đã sửa gì trước 5 phiên thật | _(TODO)_ |
