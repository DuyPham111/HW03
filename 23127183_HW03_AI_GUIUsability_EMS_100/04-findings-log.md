...
| `SV-B2-02` | Screen B2 | Bug | Tên vai trò hiển thị **"Người dự"** bằng tiếng Việt trong khi toàn bộ giao diện đang ở tiếng Anh | 1. Đặt ngôn ngữ EN<br>2. Mở một sự kiện, đọc khối Student roles<br>**Heuristic:** N4 consistency | **2** | Dữ liệu do người dùng nhập (tên vai trò) cần có bản dịch song ngữ, hoặc nêu rõ quy ước là giữ nguyên gốc | [KS_B2_workshop-b-het-cho.png](evidence/survey/KS_B2_workshop-b-het-cho.png) | |
...
| `SV-B2-08` | Screen B2 | Bug | Sau khi huỷ, khối Registration roles **trở về đúng trạng thái như chưa từng đăng ký** (checkbox bỏ tick, nút `Register (Student)` xuất hiện trở lại) — giao diện mời người dùng đăng ký lại, nhưng theo quan sát thì **không đăng ký lại được**. ⚠️ *Cần xác nhận lại chính xác biểu hiện khi bấm* | 1. Đăng ký một sự kiện<br>2. Huỷ đăng ký<br>3. Tick lại role và bấm `Register (Student)`<br>**Heuristic:** N1 visibility of system status | **3** | Nếu đúng là không cho đăng ký lại thì phải nêu rõ ngay tại chỗ và vô hiệu hoá nút, thay vì hiện giao diện như còn đăng ký được | [KS_B2_cancel-03_sau-huy.png](evidence/survey/KS_B2_cancel-03_sau-huy.png) | |
...
| `SV-B4-06` | Screen B4 — My Profile | Bug | Tên hiển thị của tài khoản là **một địa chỉ email**, và là email **khác** với email ghi ngay bên dưới ở ô EMAIL. Ô STUDENT ID hiện mã nội bộ `G69FC9C62` thay vì một mã số sinh viên. ⚠️ *Cần xác nhận tên này do bạn tự nhập hay hệ thống tự sinh* | 1. Đăng nhập tài khoản guest<br>2. Avatar → View profile<br>3. So sánh dòng tên với ô EMAIL<br>**Heuristic:** N2 · N4 | **2** | Chặn không cho dùng email làm tên hiển thị; nếu chưa có tên thì hiện phần trước dấu `@` kèm gợi ý cập nhật hồ sơ | [KS_B4_empty.png](evidence/survey/KS_B4_empty.png) | |
...
| `CL-B2-01` | Screen B2 — Trang chi tiết sự kiện | Bug | _(dòng mẫu giữ lại làm khuôn — điền khi chạy Task 1B)_ | 1. …<br>2. …<br>**Heuristic:** N? | | _(thay đổi cụ thể, không phải mục tiêu chung chung)_ | [Xem ảnh](evidence/task1b/CL-B2-01.png) | |
...
| `SV-` Khảo sát EMS | **10** | **17** | **27** | _(TODO)_ |
| **Tổng** | **10** | **17** | **27** | |
...
|---|:--:|:--:|:--:|:--:|:--:|:--:|
| Số finding | 0 | **9** | **11** | **7** | 0 | **27** |

Không có finding severity 4. Đúng như kỳ vọng: EMS đang chạy được, không có luồng nào bị chặn hoàn toàn hay mất dữ liệu. Chín finding severity 3 tập trung vào ba nhóm — **không tìm được vé sau khi đăng ký** (`SV-B4-01`, `SV-B2-09`), **thao tác không hoàn tác được nhưng cảnh báo mơ hồ** (`SV-B2-07`, `SV-B2-08`), và **hệ thống nói sai về chính nó** (`SV-ADM-02`, `SV-ADM-03`, `SV-ADM-04`).
...
| B2 Trang chi tiết sự kiện *(gồm cả khối đăng ký)* | **10** | 3 |
| B4 My Profile — QR Code + My Activities | **6** | 3 |
