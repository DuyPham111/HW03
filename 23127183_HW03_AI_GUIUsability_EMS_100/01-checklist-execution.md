...
| G-08 | Loading có skeleton/spinner, bố cục không nhảy khi data về | IA-01 | | Passed | | S2 (Slow 3G) = có spinner rõ ràng giữa trang trắng trong lúc tải (`G-08-S2.png`), không phải skeleton (khung xám dựng sẵn theo bố cục) nhưng vẫn thoả "có skeleton **hoặc** spinner". ⚠️ Chưa quan sát trực tiếp lúc nội dung ập vào có bị nhảy bố cục không (ảnh chỉ chụp được lúc đang loading) | | [G-08-S2.png](evidence/task1b/G-08-S2.png) |
...
| N-02 | Trang chi tiết có đường quay lại danh sách (không chỉ dựa Back) | IA-03 | | Passed | | ✅ **Sửa lại 06/08 — kết luận trước sai.** Nút `Back to events` CÓ tồn tại ở đầu trang B2 khi đã đăng nhập (`G-06-S2.png`, chụp trong lúc chạy Task 1B). Đã sửa `SV-B2-03` thành severity 0 trong findings log | | [G-06-S2.png](evidence/task1b/G-06-S2.png) |
| N-03 | Back trình duyệt giữ nguyên bộ lọc/trang/vị trí cuộn | IA-03 | Failed | | | S1 = Failed **cả 2 phần** của item, không chỉ filter: bấm Back đưa về **đầu trang mặc định**, vị trí cuộn cũng không giữ, cùng lúc với việc bộ lọc mất sạch (`CL-B1-02`) | S1: `CL-B1-02` | [N-06-S1-before.png](evidence/task1b/N-06-S1-before.png) · [N-06-S1-after.png](evidence/task1b/N-06-S1-after.png) |
| N-04 | Sau đăng nhập quay lại đúng trang vừa yêu cầu, không về home | IA-03 | | Failed | | S2 = đăng nhập xong **bị đẩy về trang chủ**, không quay lại đúng sự kiện vừa yêu cầu — người dùng phải tự tìm lại từ đầu | S2: `CL-B2-03` | |
...
| S-08 | Tác vụ chạy lâu có progress/spinner kèm chỉ báo bước/thời gian | IA-04 | | | N/A | S3 = N/A — Export chạy quá nhanh để tính là "tác vụ chạy lâu" (bấm là chuyển trang và tải file gần như ngay), không có gì để chấm progress/spinner ở đây. ⚠️ Ghi chú phụ (không phải lỗi): bấm Export **chuyển sang trang mới** rồi mới hiện file tải về, hơi khác kiểu tải trực tiếp thường thấy — nếu muốn soi kỹ có thể kiểm lại ở Task 3 | | |
...
| S-10 | Bộ đếm/nhãn trạng thái thời gian khớp dữ liệu ngày giờ cạnh nó | IA-04 | | | Passed | S3 = badge `Upcoming`/`Ongoing` trên thẻ hoạt động khớp đúng với khoảng ngày giờ hiển thị cạnh nó | | |
| S-11 | Mất mạng báo rõ, cho biết thao tác vừa rồi có lưu không | IA-04 | | Failed | Failed | S2 = bấm Register khi offline, spinner hiện rồi treo vô thời hạn, không báo gì. S3 = Export thất bại im lặng; Filters cũng spinner treo như S2 | S2/S3: `CL-B4-04` | |
